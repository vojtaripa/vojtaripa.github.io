...
//SHOULD NOT BE CHANGED! //$DateAdded = date("Y-m-d");
...
$statement2->execute();
$myusernameX = $statement2->fetchAll();//$statement2->mysql_result( $statement2, 0 );
$statement2->closeCursor();

foreach ($myusernameX as $myusernameX) : 
		$myquery = ($myusernameX[0]+1); 
endforeach; 


//echo "My Query: $myquery Index: $Index";

//OF COURSE ITS ALREADY BEEN ADDED... 
/*if($Index < $myquery)
{
...
$statement6->execute();
$myusername2 = $statement6->fetchAll();//$statement2->mysql_result( $statement2, 0 );
$statement6->closeCursor();

$myPlaceCount=0;
$myPlaceTotal=0;
foreach ($myusername2 as $myusername2) : 
		if($myusername2['Points']>$Points){
			$myPlaceCount++;
		    $myPlaceTotal++;
		}
		else
...
else 
{
	...
	//$query = "UPDATE `$myusername` SET (`Index`, `Date`, `Race`, `Time`, `Distance`, `Place`, `Pace`, `Location`, `Type`, `Picture`, `LinkToResults`, `LinkToActivity`, `shoes`, `Notes`, `Points`, `Modified`, `DateAdded`) VALUES ( '" . $Index . "','" . $Date . "','" . $Race . "','" . $Time . "','" . $Distance . "','" . $Place . "','" . $Pace . "','" . $Location . "','" . $Type . "','" . $filename . "','" . $LinkToResults . "','" . $LinkToActivity . "','" . $shoes . "','" . $Notes . "','" . $Points . "','" . $Modified . "','" . $DateAdded . "')";
	
	$query = "UPDATE `$myusername` SET `Date` = '" . $Date . "', `Race`= '" . $Race . "', `Time`= '" . $Time . "', `Distance`= '" . $Distance . "', `Place`= '" . $Place . "', `Pace`= '" . $Pace . "', `Location`= '" . $Location . "', `Type` = '" . $Type . "', `Picture`= '" . $filename . "', `LinkToResults`= '" . $LinkToResults . "', `LinkToActivity`= '" . $LinkToActivity . "', `shoes`= '" . $shoes . "', `Notes`= '" . $Notes . "', `Points`= '" . $Points . "', `Modified`= '" . $Modified . "', `Feel`= '" . $Feel . "' WHERE `$myusername`.`Index` = $Index";
	
	//echo "Query: $query<br><br>";
	//    ex. UPDATE `$myusername`(`Index`, `Date`, `Race`, `Time`, `Distance`, `Place`, `Pace`, `Location`, `Type`, `Picture`, `LinkToResults`, `LinkToActivity`, `shoes`, `Notes`, `Points`, `Modified`, `DateAdded`) VALUES ( '93','2017-07-04','Kenwood Footrace 10k 2017','0:35:13','6.20','9','00:05:40','Kenwood, Ca','Road','','https://results.chronotrack.com/event/results/event/event-27628','https://www.strava.com/activities/1067446413','Hoka Trace','Good Race, hilly race, started feeling sluggish at 4mi but did hang with 2 guys infront of me and out-kicked one of them','568','','2017-07-13')
//working ex. UPDATE `$myusername` SET `Notes` = 'Good Race, hilly race, started feeling sluggish at 4mi but did hang with 2 guys infront of me and out-kicked one of them.\r\nFastest time yet on this course!' WHERE `$myusername`.`Index` = 93;
//"$sql   = \'UPDATE `$myusername` SET `Notes` =\'Good Race, hilly race, started feeling sluggish at 4mi but did hang with 2 guys infront of me and out-kicked one of them.\\r\\nFastest time yet on this course!\' WHERE `$myusername`.`Index` = 93"';	
//multiple    UPDATE `$myusername` SET `Notes` = 'Good Race, hilly race, started feeling sluggish at 4mi but did hang with 2 guys infront of me and out-kicked one of them.\r\nFastest time yet on this course!!', `Points` = '1' WHERE `$myusername`.`Index` = 93;
    ...
    */
    ...
    echo "<center><br><br><tr><th><h2> * MODIFIED RACE: $Race!* </th></tr></h2>
